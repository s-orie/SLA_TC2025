...
def ScanLineMask(image):
    sarea = image.select('B1')
    sarea = sarea.multiply(0).add(1)
    #scan line detect
    edge = ee.Algorithms.CannyEdgeDetector(sarea,1,1)
    connected = edge.mask(edge).lt(0.8).connectedPixelCount(35, True)
    edgeLong = connected.gte(35)
    edge = edgeLong
    line = sarea.mask(edge.focal_max(30, 'square', 'meters')).rename('ScanLine')
    #mask
    mask = sarea.where(line,0)
    return mask
...
print('Number of Images (Landsat 5): ', ls5.size().getInfo())
...
ls5_NDSI = ls5.map(NDSI)
SnowArea_ls5 = ls5_NDSI.map(SnowAreaDetect)
Edges = SnowArea_ls5.map(EdgeDetect)
